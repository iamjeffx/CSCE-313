...
public:
    SHMQ(string _name, int _len) : name(_name), len(_len) {
        ...
        rd = sem_open((char *)(name + "rd").c_str(), O_CREAT, 0600, 1);
        sd = sem_open((char *)(name + "sd").c_str(), O_CREAT, 0600, 0);
    }
    ~SHMQ() {
        sem_close(sd);
        sem_close(rd);
        sem_unlink((char *)(name + "rd").c_str());
        sem_unlink((char *)(name + "sd").c_str());

        munmap(segment, len);
        shm_unlink((char *)name.c_str());
    }
...
private:
    ...
    int len;
public:
    SHMRequestChannel(const string _name, const Side _side, int len);
